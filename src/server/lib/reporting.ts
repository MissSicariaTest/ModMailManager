...
export function createEmptyDailyReportStore(): DailyReportStore {
  return {
    ...
    subreddits: {
      primary: emptyMetrics(),
      secondary: emptyMetrics(),
    },
    modmailConversations: {},
    trackedPosts: {},
    ticketHandlers: {
      primary: emptyHandlerStats(),
      secondary: emptyHandlerStats(),
    },
  };
}
...
export async function getDailyReportStore(): Promise<DailyReportStore> {
  ...
  return {
    ...
    ...stored,
    subreddits: {
      primary: { ...emptyMetrics(), ...stored.subreddits?.primary },
      secondary: { ...emptyMetrics(), ...stored.subreddits?.secondary },
    },
    modmailConversations: stored.modmailConversations ?? {},
    trackedPosts: stored.trackedPosts ?? {},
    ticketHandlers: {
      primary: { ...emptyHandlerStats(), ...stored.ticketHandlers?.primary },
      secondary: {
        ...emptyHandlerStats(),
        ...stored.ticketHandlers?.secondary,
      },
    },
  };
}
