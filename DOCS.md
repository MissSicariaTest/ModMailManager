...
| `CLOSED_TICKETS_WEBHOOK_PRIMARY` | Full Discord webhook URL for your closed-tickets channel (Webhook 7) — required for closed-ticket moves |
| `CLOSED_TICKETS_WEBHOOK_SECONDARY` | *(Optional)* Full webhook URL for the secondary closed-tickets channel (Webhook 8) |
| `REPORTING_WEBHOOK_PRIMARY` | *(Optional)* Reporting-channel webhook for the primary subreddit — enables its daily ticket-actions report at ~8:05 AM Eastern |
| `REPORTING_WEBHOOK_SECONDARY` | *(Optional)* Reporting-channel webhook for the secondary subreddit |
...
Click **Save Changes**. Also set the same Webhook 7 URL as the `CLOSED_TICKETS_WEBHOOK_PRIMARY` secret on the Cloudflare Worker — the Worker performs the actual move when tickets close.
...
- **Worker → Discord:** the Worker edits embeds, moves closed tickets to the archive channel (using the `CLOSED_TICKETS_WEBHOOK_PRIMARY` secret), and posts its own **Daily Ticket Report** to your reporting channel on a schedule (using the `REPORTING_WEBHOOK` secret and Cloudflare cron triggers).
...
- Removed all community-identifying names from source code, configuration keys, secret names, and the public health endpoint. Internal routing now uses neutral `primary` / `secondary` group keys throughout.
- Cloudflare secret names changed: `CLOSED_TICKETS_WEBHOOK_PRIMARY`, `CLOSED_TICKETS_WEBHOOK_SECONDARY`, `REPORTING_WEBHOOK_PRIMARY`, `REPORTING_WEBHOOK_SECONDARY`, plus new optional `SECONDARY_SUBREDDIT_NAME` for routing a second community's tickets.
- Reddit app setting keys renamed to `primary*` / `secondary*` — webhook URLs must be re-entered in app settings after upgrading.
