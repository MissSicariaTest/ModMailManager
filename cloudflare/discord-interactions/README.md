...
1. A Discord application with a bot token (see [full documentation](../../DOCS.md#create-a-discord-bot))
...
| Secret | Purpose |
| --- | --- |
| `CLOSED_TICKETS_WEBHOOK_PRIMARY` | Full Discord webhook URL for your primary closed-tickets channel (Webhook 7) |
| `CLOSED_TICKETS_WEBHOOK_SECONDARY` | Full Discord webhook URL for your secondary closed-tickets channel (Webhook 8) — only if monitoring two subreddits |
| `SECONDARY_SUBREDDIT_NAME` | *(Optional)* Name of your second subreddit (without r/). Tickets from that subreddit route to the SECONDARY webhooks above. |

Daily ticket report (optional — set at least one):

| Secret | Purpose |
| --- | --- |
| `REPORTING_WEBHOOK_PRIMARY` | Discord webhook URL for the **primary** subreddit's reporting channel. Receives that subreddit's Daily Ticket Report. |
| `REPORTING_WEBHOOK_SECONDARY` | Discord webhook URL for the **secondary** subreddit's reporting channel. |
...
Webhook 7 is still needed in the Reddit app settings so the Reddit app knows to tag each ticket with the closed-channel destination when it sends the initial alert. The actual move is performed by the Cloudflare Worker using `CLOSED_TICKETS_WEBHOOK_PRIMARY` — both must be set to the same webhook URL.
...
- `CLOSED_TICKETS_WEBHOOK_PRIMARY: true` — primary closed-tickets webhook is set ✓
