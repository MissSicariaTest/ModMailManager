...
export interface Env {
  ...
  CLOSED_TICKETS_WEBHOOK_PRIMARY?: string;
  CLOSED_TICKETS_WEBHOOK_SECONDARY?: string;
  REPORTING_WEBHOOK?: string;
  REPORTING_WEBHOOK_PRIMARY?: string;
  REPORTING_WEBHOOK_SECONDARY?: string;
  SECONDARY_SUBREDDIT_NAME?: string;
}
...
type ClosedWebhookConfig = {
  primary?: ClosedWebhookCredentials | null;
  secondary?: ClosedWebhookCredentials | null;
  syncedAt?: string;
};
...
function resolveReportingWebhook(env: Env, subreddit: string): string | null {
  const specific =
    subreddit === "secondary"
      ? env.REPORTING_WEBHOOK_SECONDARY
      : env.REPORTING_WEBHOOK_PRIMARY;
...
async function maybeSendDailyTicketReport(env: Env): Promise<void> {
  const anyWebhook =
    ...
    env.REPORTING_WEBHOOK_PRIMARY?.trim() ||
    env.REPORTING_WEBHOOK_SECONDARY?.trim();
  if (!anyWebhook) {
    return;
  }
...
subreddit's report goes to REPORTING_WEBHOOK_PRIMARY or
REPORTING_WEBHOOK_SECONDARY, with REPORTING_WEBHOOK as a
shared fallback.
...
function getHealthStatus(env: Env): Record<string, unknown> {
  const primaryWebhookSet = isSecretConfigured(env.CLOSED_TICKETS_WEBHOOK_PRIMARY);
  const primaryWebhookValid = primaryWebhookSet
    ? parseWebhookUrl(env.CLOSED_TICKETS_WEBHOOK_PRIMARY ?? "") !== null
    : false;
  const secondaryWebhookSet = isSecretConfigured(env.CLOSED_TICKETS_WEBHOOK_SECONDARY);
  const secondaryWebhookValid = secondaryWebhookSet
    ? parseWebhookUrl(env.CLOSED_TICKETS_WEBHOOK_SECONDARY ?? "") !== null
    : false;
  ...
  return {
    ...
    secrets: {
      ...
      CLOSED_TICKETS_WEBHOOK_PRIMARY: primaryWebhookSet,
      CLOSED_TICKETS_WEBHOOK_PRIMARY_valid: primaryWebhookValid,
      CLOSED_TICKETS_WEBHOOK_SECONDARY: secondaryWebhookSet,
      CLOSED_TICKETS_WEBHOOK_SECONDARY_valid: secondaryWebhookValid,
      REPORTING_WEBHOOK_PRIMARY: isSecretConfigured(env.REPORTING_WEBHOOK_PRIMARY),
      REPORTING_WEBHOOK_SECONDARY: isSecretConfigured(
        env.REPORTING_WEBHOOK_SECONDARY
      ),
      REPORTING_WEBHOOK: isSecretConfigured(env.REPORTING_WEBHOOK),
      SECONDARY_SUBREDDIT_NAME: isSecretConfigured(env.SECONDARY_SUBREDDIT_NAME),
    },
    note: "CLOSED_TICKETS_WEBHOOK_PRIMARY_valid must be true for closed-ticket archiving to work. If set but not valid, the URL format is wrong — it must be a full Discord webhook URL containing /api/webhooks/.",
  };
}
...
async function syncClosedWebhooksConfig(request: Request, env: Env): Promise<Response> {
  ...
  return Response.json({
    ...
    closedWebhooksFromReddit: {
      primary: Boolean(merged.primary?.webhookId),
      secondary: Boolean(merged.secondary?.webhookId),
    },
...
async function handleButton(
  ...
  ctx.waitUntil(
    trackAction(env, normalizeSubredditGroup(updated.subreddit, env), parsed.action, actor.username).catch((error) => {
...
async function handleModal(
  ...
  ctx.waitUntil(
    trackAction(env, normalizeSubredditGroup(updated.subreddit, env), "reassign", actor.username).catch((error) => {
      console.error("Failed to track ticket action:", error);
    })
  );
...
/**
 * Maps a ticket's stored subreddit value (or an embed's Subreddit field) to a
 * routing group. Legacy tickets stored community-specific keys; normalize
 * them to primary/secondary. The optional SECONDARY_SUBREDDIT_NAME secret
 * lets operators route a second community's tickets to the secondary group.
 */
function normalizeSubredditGroup(value: string, env?: Env): "primary" | "secondary" {
  const normalized = value.replace(/^r\//i, "").trim().toLowerCase();
  if (normalized === "secondary") {
    return "secondary";
  }
  // Legacy ticket records and embed Subreddit fields carry the real
  // subreddit name; operators map their second community via the
  // SECONDARY_SUBREDDIT_NAME secret.
  const secondaryName = env?.SECONDARY_SUBREDDIT_NAME?.replace(/^r\//i, "").trim().toLowerCase();
  if (secondaryName && normalized === secondaryName) {
    return "secondary";
  }
  return "primary";
}
...
function bootstrapTicketFromInteraction(
  ...
  return {
    id: ticketId,
    source: "modmail",
    subreddit: normalizeSubredditGroup(subredditField?.value ?? "", env),
    status,
    assignedTo,
    webhookId: "",
    webhookToken: "",
...
async function resolveTicketForInteraction(
  ...
  const bootstrapped = bootstrapTicketFromInteraction(interaction, ticketId, env);
  if (!bootstrapped) {
    return null;
  }
...
async function trackAction(
  ...
  if (!snapshot.subreddits[subreddit]) {
    snapshot.subreddits[subreddit] = createEmptySnapshot().subreddits.primary;
  }

  const metrics = snapshot.subreddits[subreddit];
  const metricKeyMap: Partial<Record<TicketAction, keyof typeof metrics>> = {
  ...
  const metricKey = metricKeyMap[action];

  if (metricKey && typeof metrics[metricKey] === "number") {
...
async function mergeClosedWebhookFromTicket(env: Env, ticket: TicketRecord): Promise<void> {
  ...
  const group = normalizeSubredditGroup(ticket.subreddit, env);
  const existing = (await getClosedWebhookConfig(env)) ?? {};
  const merged: ClosedWebhookConfig = {
    ...existing,
    [group]: {
...
async function getHealthStatusWithSync(env: Env): Promise<Response> {
  ...
  return Response.json({
    ...getHealthStatus(env),
    closedWebhooksFromReddit: {
      primary: Boolean(kvConfig?.primary?.webhookId),
      secondary: Boolean(kvConfig?.secondary?.webhookId),
    },
...
function getClosedWebhookFromEnv(
  ...
  const envUrl =
    subreddit === "secondary"
      ? env.CLOSED_TICKETS_WEBHOOK_SECONDARY
      : env.CLOSED_TICKETS_WEBHOOK_PRIMARY;
  if (!envUrl?.trim()) {
    return null;
  }
